...
**Background:** Molecular representation choice is a central design decision in property prediction, yet comparisons are often confounded by inconsistent splits, undocumented curation, and evaluation against a single model family. We present MolFusion, a reproducible framework for systematic comparison of heterogeneous molecular representations under a frozen protocol, in which datasets, splits, hyperparameter budgets and statistical procedures are fixed in advance and every result carries a content-derived identity.

**Methods:** We benchmarked seven fixed-vector representations — circular, substructure and substructure-key fingerprints, physicochemical and fragment-count descriptors, a reduced-graph encoding, and a SMILES n-gram TF-IDF — across 22 ADMET endpoints from the Therapeutics Data Commons, using a regularized linear probe and a gradient-boosting probe with the endpoint as the unit of statistical inference.

**Results:** Under the nonlinear probe, a compact 217-dimensional physicochemical descriptor representation achieved the strongest cross-endpoint mean ranking (1.91 on the official partition), supported by a Friedman omnibus test, Holm-corrected paired Wilcoxon comparisons, and large matched-pairs rank-biserial effect sizes. Under the linear probe, no representation separated clearly from the field, and the highest-ranked representation differed between evaluation tracks. The nonlinear ranking was reproduced under independently generated Bemis–Murcko scaffold partitions with stricter curation (mean rank 1.77), whereas the linear result remained unresolved. These findings indicate that representation comparisons are contingent on the downstream model family, and that reporting a representation ranking without specifying the probe is insufficient.

**Keywords:** molecular representation; ADMET prediction; molecular fingerprints; physicochemical descriptors; scaffold split; benchmarking; reproducibility; cheminformatics

<!-- Graphical abstract optional: 920 x 300 px, max 150 KB, jpeg/png/svg. Specification in metadata/GRAPHICAL_ABSTRACT_SPEC.md; artwork not generated. -->
...
[@morgan1965; @rogers2010]. Substructure key fingerprints record
the presence or absence of a predefined list of chemical patterns
[@durant2002], and related substructure fingerprints enumerate
paths and features over the molecular graph [@gedeck2006].
...
[@todeschini2009]. Fragment-count descriptors count
...
scaffold-level similarity [@stiefl2006]. Representations derived from
the SMILES line notation [@weininger1988] treat the molecule as a string
...
[@krenn2020]. A further class of learned representations —
...
specifying it in advance [@david2020].
...
a heterogeneous set of endpoints [@ceretomassague2015]. Two
...
[@friedman1937; @demsar2006]. Applying them together, and recording enough
...
and partitions [@huang2021; @huang2022], which makes comparability with
...
[@pedregosa2011], under an identical hyperparameter budget, so that
...
scaffold [@bemis1996] under stricter curation. The two
...
[@krenn2020], which was deliberately **excluded from Track A**.
...
with RDKit [@rdkit] version 2026.03.5, and each was frozen at
...
**Morgan / ECFP.** Circular fingerprints [@morgan1965; @rogers2010] computed with **radius 2** and folded to **1024 bits**, corresponding
...
structural key set [@durant2002], a binary vector in which each
...
**Avalon.** The Avalon fingerprint [@gedeck2006] at **1024 bits**,
...
[@stiefl2006] with the frozen parameters **`atomTypes = 0`,
...
reference corpus [@zdrazil2024]. Vocabulary selection was
...
SMILES** [@weininger1988] under the frozen normalisation contract
...
[@huang2021; @huang2022], comprising **22 endpoints**: **13
...
## Molecular standardization, identity, and data curation
...
## Track A1 — official TDC evaluation
...
## Track A2 — independent scaffold robustness evaluation
...
scaffold [@bemis1996], using seeds **0–4**. Scaffolds
...
[@pedregosa2011]. Logistic regression was fitted with an iteration
...
[@friedman1937; @demsar2006] was applied to the within-endpoint ranks of the
...
[@wilcoxon1945] over endpoint-level scores, with
**Holm correction** [@holm1979] applied across the pairwise
family. Every pairwise result is reported with its **matched-pairs
rank-biserial correlation** [@kerby2014] as an
...
characterised by a nonparametric bootstrap [@efron1979] with
**10,000 resamples**, a
...
[@kendall1939] was computed across the five partitions for each
...
Statistical computation used SciPy [@virtanen2020] and scikit-learn.
...
## Reproducibility, result identity, and provenance
...
## Benchmark coverage and representation characteristics
...
## Linear-probe representation performance
...
## Nonlinear-probe representation performance
...
## Robustness under independent scaffold repartitioning
...
## Representation stability across endpoints
...
## Representation utility is probe-dependent
...
## Compact physicochemical descriptors under nonlinear probing
...
quantities [@todeschini2009]. Fingerprint representations
...
known to affect downstream performance [@ceretomassague2015].
...
[@david2020]; they were not part of this benchmark
...
[@todeschini2009] as directly interpretable continuous
axes, which a tree-based model can threshold individually, whereas
substructure-presence encodings [@ceretomassague2015] express
...
## Robustness across official and repartitioned evaluation
...
## Implications for molecular representation benchmarking
...
[@david2020]. Standardised benchmark collections
have substantially improved comparability by fixing datasets, metrics and
splits [@huang2021; @huang2022], and earlier benchmarking work established
...
[@wu2018]. A benchmark constrains only what it
...
[@zdrazil2024] before any endpoint evaluation and without access to
...
[@bemis1996] and applies stricter curation. The two
...
probe × task families whose Friedman omnibus test [@friedman1937; @demsar2006]
rejected at α = 0.05, using **paired Wilcoxon signed-rank tests over
endpoint-level scores** [@wilcoxon1945] with **Holm
correction applied within each family** [@holm1979]. **The
...
correlation as an effect size [@kerby2014], the
representation
